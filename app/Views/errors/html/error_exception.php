...
<html>
  ...
  <body onload="init()">
    ...
    <div class="header">
      <div class="container">
        <h1>
          <?= htmlspecialchars($title, ENT_SUBSTITUTE, 'UTF-8'), ($exception->getCode() ? ' #' . $exception->getCode() : '') ?>
        </h1>
        <p>
          <?= str_replace(ZOOPATH,"/ZOOPATH/",$exception->getMessage()) ?>
            ...
            <li>
              <p>
                ...
                <?php
								if (isset($row['function']) && in_array($row['function'], ['include', 'include_once', 'require', 'require_once']))
									...
									{
									echo str_replace(ZOOPATH,"/ZOOPATH/",static::cleanPath($row['file'])) . ' : ' . $row['line'];
								}
								?>
                <?php else : ?>
                {PHP internal code}
              ...
              <div class="args" id="<?= $args_id ?>">
                <table cellspacing="0">
                  ...
                  <tr>
                    <td>
                      <code><?= htmlspecialchars(isset($params[$key]) ? '$' . $params[$key]->name : "#$key", ENT_SUBSTITUTE, 'UTF-8') ?></code>
                    </td>
                    <td>
