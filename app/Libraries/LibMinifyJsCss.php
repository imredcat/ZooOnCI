...
{
  public $arr_is_force = [];        // 캐시를 생성하지 않을 경우 $this->loadjscss(['path/script.js:force'])
  public $use_original = 0;         // 전체 호출된 스크립트,스타일을 원본 그대로 호출
  ...
  public function useOriginal($use_original=1)
  {
   ...
   * js, css 파일을 가져온다.
   */
  public function loadjscss($arr_jscss=array(), $withmodule=1,$use_original=null)
  {
    if (!is_null($use_original)) {
    $this->useOriginal($use_original);
    }
    $this->gcCaches();
  ...
  public function getRealPath(string $file_path)
  {
    ...
    if ($arr_path[0] == '_mod_') {
      ...
      $file_path = str_replace('//','/',join("/", $arr_path));
    } else {
  ...
  public function getSrcUrl($arr, $filetype)
  {
    ...
    if ($this->use_original == 1) {
      foreach ($arr_path_real as $tagtype => $_arr_path) {
        foreach ($_arr_path as $key => $_file) {
          $mtime = filemtime($_file);
          $arr_path_called[$tagtype][$key] = [$arr_path_called[$tagtype][$key],$mtime];
        }
      }
      return $arr_path_called;
    }
